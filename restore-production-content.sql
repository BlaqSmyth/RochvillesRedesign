-- ============================================
-- RESTORE ORIGINAL CONTENT TO PRODUCTION
-- Run this in Supabase SQL Editor
-- ============================================

-- Step 1: Clear existing content (handle foreign keys)
...
INSERT INTO payroll_packages (name, employee_range, price, features, display_order) VALUES
...
INSERT INTO business_types (type, icon, popular, display_order) VALUES 
('Sole Trader', 'Briefcase', false, 1),
('Partnership', 'Users', false, 2),
('Limited Company', 'Building2', true, 3),
('Solicitors', 'Scale', false, 4);

-- Sole Trader Tiers
INSERT INTO pricing_tiers (business_type_id, turnover, price, display_order)
SELECT id, 'Up to £25k', '£550', 1 FROM business_types WHERE type = 'Sole Trader'
UNION ALL SELECT id, '£26k - £35k', '£650', 2 FROM business_types WHERE type = 'Sole Trader'
UNION ALL SELECT id, '£35k - £100k', '£1,000', 3 FROM business_types WHERE type = 'Sole Trader'
UNION ALL SELECT id, 'Above £100k', 'POA', 4 FROM business_types WHERE type = 'Sole Trader'
-- Partnership Tiers
UNION ALL SELECT id, 'Up to £25k', '£650', 1 FROM business_types WHERE type = 'Partnership'
UNION ALL SELECT id, '£26k - £35k', '£750', 2 FROM business_types WHERE type = 'Partnership'
UNION ALL SELECT id, '£35k - £100k', '£1,250', 3 FROM business_types WHERE type = 'Partnership'
UNION ALL SELECT id, 'Above £100k', 'POA', 4 FROM business_types WHERE type = 'Partnership'
-- Limited Company Tiers
UNION ALL SELECT id, 'Up to £25k', '£850', 1 FROM business_types WHERE type = 'Limited Company'
UNION ALL SELECT id, '£26k - £35k', '£1,000', 2 FROM business_types WHERE type = 'Limited Company'
UNION ALL SELECT id, '£35k - £100k', '£1,850', 3 FROM business_types WHERE type = 'Limited Company'
UNION ALL SELECT id, 'Above £100k', 'POA', 4 FROM business_types WHERE type = 'Limited Company'
-- Solicitors Tiers
UNION ALL SELECT id, 'Up to £25k', '£1,250', 1 FROM business_types WHERE type = 'Solicitors'
UNION ALL SELECT id, '£26k - £35k', '£1,500', 2 FROM business_types WHERE type = 'Solicitors'
UNION ALL SELECT id, '£35k - £100k', '£2,300', 3 FROM business_types WHERE type = 'Solicitors'
UNION ALL SELECT id, 'Above £100k', 'POA', 4 FROM business_types WHERE type = 'Solicitors';

-- Step 6: Restore Additional Services
INSERT INTO additional_services (service_name, price, period, description, icon, display_order) VALUES
...
SELECT 'RESTORATION COMPLETE!' as status,
       ...
       (SELECT COUNT(*) FROM payroll_packages) as payroll_packages,
       (SELECT COUNT(*) FROM business_types) as business_types,
       (SELECT COUNT(*) FROM pricing_tiers) as pricing_tiers,
       (SELECT COUNT(*) FROM additional_services) as additional_services;
