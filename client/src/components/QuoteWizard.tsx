...
import { ChevronLeft, ChevronRight, Check, Calculator, Rocket } from "lucide-react";
...
export default function QuoteWizard({ embedded = false }: QuoteWizardProps) {
  ...
  const createQuoteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      if (!embedded) {
        ...
        // Reset wizard for embedded version
        setCurrentStep(1);
        setStep1Data(null);
        setStep2Data(null);
        step1Form.reset();
  ...
  return (
    <motion.div
      ...
      className={embedded ? "" : "min-h-screen bg-background"}
    >
      <div className={embedded ? "" : "container py-12 max-w-4xl"}>
        {!embedded && (
          <div className="mb-8">
        ...
        <Card className={embedded ? "bg-gradient-to-br from-primary/5 via-background to-primary/10 border-2 border-primary/20" : ""}>
          <CardHeader>
            <CardTitle>
                ...
                <Card className="bg-muted/50">
                  ...
                  <CardContent className="space-y-3">
                    <div className="flex justify-between">
                      ...
                      <span className="font-semibold" data-testid="text-base-price">£{calculation.basePrice.toFixed(2)}</span>
                    </div>
                    {calculation.payrollPrice > 0 && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Payroll Package</span>
                        <span className="font-semibold" data-testid="text-payroll-price">£{calculation.payrollPrice.toFixed(2)}</span>
                      </div>
                    )}
                    {calculation.additionalServicesTotal > 0 && (
                      <div className="flex justify-between">
                        <span className="text-muted-foreground">Additional Services</span>
                        <span className="font-semibold" data-testid="text-additional-price">£{calculation.additionalServicesTotal.toFixed(2)}</span>
                      </div>
                    )}
                    <div className="flex justify-between pt-2 border-t">
                      <span className="text-muted-foreground">Subtotal</span>
                      <span className="font-semibold" data-testid="text-subtotal">£{calculation.subtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-muted-foreground">VAT (20%)</span>
                      <span className="font-semibold" data-testid="text-vat">£{calculation.vatAmount.toFixed(2)}</span>
                    </div>
